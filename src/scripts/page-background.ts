...
class PageBackground {
	...
	/**
	 * Stops the background from rendering by canceling the animation frame.
	 */
	public stopRendering = () => {
		if (this.animationFrameId !== null) {
			cancelAnimationFrame(this.animationFrameId);
			this.animationFrameId = null;
			// Clears the background of any in-progress animations
			this.overlayCtx.clearRect(
				0,
				0,
				this.overlayCanvas.width,
...
async function initializeBackground() {
	...
	// Store the rendering state
	if (!("isRendering" in localStorage)) {
		localStorage.setItem("isRendering", "true");
	}
	const isRendering = localStorage.getItem("isRendering");
